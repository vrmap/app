...
function ProcessExcel(data) {
    ...
    //var FamiglieConFigli = famiglie.filter((f) => f.figli.length > 0);
    //console.log(FamiglieConFigli)
    var soglia = document.getElementById("soglia").value;
    //console.log(soglia)
    var filtro_data = moment(soglia, "DD/MM/YYYY").toDate();


    if (soglia.length > 0) Famiglie_dopo_scadenza = famiglie.filter((f) => f.scadenza >= filtro_data);
    else
